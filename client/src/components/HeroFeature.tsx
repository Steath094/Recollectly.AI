...
export default function HeroFeature() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 px-6 md:px-16 lg:px-24 py-20">
      {features.map(([element, title, description], i) => (
        <HeroFeatureCard key={i} element={element} title={title} description={description} />
      ))}
    </div>
  );
}
